...
function logDrinkHere(){ if(!curId)return; openForm(curId); setTimeout(function(){ addDrinkRow("","",localToday()); const rows=document.querySelectorAll("#f-drinks .dr"); const last=rows[rows.length-1]; if(last){ last.classList.remove("collapsed"); const n=last.querySelector(".dn"); if(n){ try{ n.focus({preventScroll:true}); }catch(e){ n.focus(); } } last.scrollIntoView({block:"center"}); } _formSnap=formSnapshot(); },140); }
